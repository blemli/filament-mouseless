...
With `->teach()` enabled, users who never discovered the overlay get a one-off "Press ? to see every shortcut" notification shortly after a page settles — same backoff, dismiss and mute as every other tip, and three opens mark it learned for good. Opt out:
